...
class FieldElement extends UnmodifiableInt32ListView {
  ...
  FieldElement operator * (other) {
    ...
    var carry0 = ((h0 + (1<<25)) as int) >> 26; h1 += carry0; h0 -= carry0 << 26;
    var carry4 = ((h4 + (1<<25)) as int) >> 26; h5 += carry4; h4 -= carry4 << 26;

    var carry1 = ((h1 + (1<<24)) as int) >> 25; h2 += carry1; h1 -= carry1 << 25;
    var carry5 = ((h5 + (1<<24)) as int) >> 25; h6 += carry5; h5 -= carry5 << 25;

    var carry2 = ((h2 + (1<<25)) as int) >> 26; h3 += carry2; h2 -= carry2 << 26;
    var carry6 = ((h6 + (1<<25)) as int) >> 26; h7 += carry6; h6 -= carry6 << 26;

    var carry3 = ((h3 + (1<<24)) as int) >> 25; h4 += carry3; h3 -= carry3 << 25;
    var carry7 = ((h7 + (1<<24)) as int) >> 25; h8 += carry7; h7 -= carry7 << 25;

    carry4 = ((h4 + (1<<25)) as int) >> 26; h5 += carry4; h4 -= carry4 << 26;
    var carry8 = ((h8 + (1<<25)) as int) >> 26; h9 += carry8; h8 -= carry8 << 26;

    var carry9 = ((h9 + (1<<24)) as int) >> 25; h0 += carry9 * 19; h9 -= carry9 << 25;
  ...
  /// invert FieldElement
  FieldElement invert() {
    var t0 = FieldElement(), t1 = FieldElement(),
  ...
  FieldElement sqrt() {
    ...
    if (!(legendre == FieldElement.zero()) && !(legendre == FieldElement.one())) {
      return null;
    }
...
class ProjectiveGroupElement {
  ...
  factory ProjectiveGroupElement.zero(){
    return ProjectiveGroupElement(FieldElement.zero(), FieldElement.one(), FieldElement.one());
  }
  ...
  ProjectiveGroupElement scalarMult(List<int> a, ExtendedGroupElement A, List<int> b) {
    ...
    t = A.dblToCompleted(); A2 = t.toExtended();
    t = A2 + Ai[0]; u = t.toExtended(); Ai[1] = u.toCached();
    t = A2 + Ai[1]; u = t.toExtended(); Ai[2] = u.toCached();
    t = A2 + Ai[2]; u = t.toExtended(); Ai[3] = u.toCached();
    t = A2 + Ai[3]; u = t.toExtended(); Ai[4] = u.toCached();
    t = A2 + Ai[4]; u = t.toExtended(); Ai[5] = u.toCached();
    t = A2 + Ai[5]; u = t.toExtended(); Ai[6] = u.toCached();
    t = A2 + Ai[6]; u = t.toExtended(); Ai[7] = u.toCached();
...
class ExtendedGroupElement {
  ...
  factory ExtendedGroupElement.zero(){
    return ExtendedGroupElement(FieldElement.zero(), FieldElement.one(), FieldElement.one(),
        FieldElement.zero());
  }
  ...
  factory ExtendedGroupElement.scalarMultBase(List<int> a) {
    ...
    r = h.dblToCompleted(); s = r.toProjective();
    r = s.dblToCompleted(); s = r.toProjective();
    r = s.dblToCompleted(); s = r.toProjective();
    r = s.dblToCompleted(); h = r.toExtended();
...
class Scalar extends UnmodifiableInt8ListView {
  ...
  List<int> reduction() {
    ...
    var carry6 = (s6 + (1<<20)) >> 21; s7 += carry6; s6 -= carry6 << 21;
    var carry8 = (s8 + (1<<20)) >> 21; s9 += carry8; s8 -= carry8 << 21;
    var carry10 = (s10 + (1<<20)) >> 21; s11 += carry10; s10 -= carry10 << 21;
    var carry12 = (s12 + (1<<20)) >> 21; s13 += carry12; s12 -= carry12 << 21;
    var carry14 = (s14 + (1<<20)) >> 21; s15 += carry14; s14 -= carry14 << 21;
    var carry16 = (s16 + (1<<20)) >> 21; s17 += carry16; s16 -= carry16 << 21;

    var carry7 = (s7 + (1<<20)) >> 21; s8 += carry7; s7 -= carry7 << 21;
    var carry9 = (s9 + (1<<20)) >> 21; s10 += carry9; s9 -= carry9 << 21;
    var carry11 = (s11 + (1<<20)) >> 21; s12 += carry11; s11 -= carry11 << 21;
    var carry13 = (s13 + (1<<20)) >> 21; s14 += carry13; s13 -= carry13 << 21;
    var carry15 = (s15 + (1<<20)) >> 21; s16 += carry15; s15 -= carry15 << 21;
    ...
    var carry0 = (s0 + (1<<20)) >> 21; s1 += carry0; s0 -= carry0 << 21;
    var carry2 = (s2 + (1<<20)) >> 21; s3 += carry2; s2 -= carry2 << 21;
    var carry4 = (s4 + (1<<20)) >> 21; s5 += carry4; s4 -= carry4 << 21;
    carry6 = (s6 + (1<<20)) >> 21; s7 += carry6; s6 -= carry6 << 21;
    carry8 = (s8 + (1<<20)) >> 21; s9 += carry8; s8 -= carry8 << 21;
    carry10 = (s10 + (1<<20)) >> 21; s11 += carry10; s10 -= carry10 << 21;

    var carry1 = (s1 + (1<<20)) >> 21; s2 += carry1; s1 -= carry1 << 21;
    var carry3 = (s3 + (1<<20)) >> 21; s4 += carry3; s3 -= carry3 << 21;
    var carry5 = (s5 + (1<<20)) >> 21; s6 += carry5; s5 -= carry5 << 21;
  ...
  List<int> mulAdd(Scalar b, Scalar c) {
    ...
    s[2]  = cCopy[2]  + aCopy[0]*bCopy[2]  + aCopy[1]*bCopy[1] + aCopy[2]*bCopy[0];
    s[3]  = cCopy[3]  + aCopy[0]*bCopy[3]  + aCopy[1]*bCopy[2] + aCopy[2]*bCopy[1]  + aCopy[3]*bCopy[0];
    s[4]  = cCopy[4]  + aCopy[0]*bCopy[4]  + aCopy[1]*bCopy[3] + aCopy[2]*bCopy[2]  + aCopy[3]*bCopy[1] + aCopy[4]*bCopy[0];
    s[5]  = cCopy[5]  + aCopy[0]*bCopy[5]  + aCopy[1]*bCopy[4] + aCopy[2]*bCopy[3]  + aCopy[3]*bCopy[2] + aCopy[4]*bCopy[1] + aCopy[5]*bCopy[0];
    s[6]  = cCopy[6]  + aCopy[0]*bCopy[6]  + aCopy[1]*bCopy[5] + aCopy[2]*bCopy[4]  + aCopy[3]*bCopy[3] + aCopy[4]*bCopy[2] + aCopy[5]*bCopy[1] + aCopy[6]*bCopy[0];
    s[7]  = cCopy[7]  + aCopy[0]*bCopy[7]  + aCopy[1]*bCopy[6] + aCopy[2]*bCopy[5]  + aCopy[3]*bCopy[4] + aCopy[4]*bCopy[3] + aCopy[5]*bCopy[2] + aCopy[6]*bCopy[1] + aCopy[7]*bCopy[0];
    s[8]  = cCopy[8]  + aCopy[0]*bCopy[8]  + aCopy[1]*bCopy[7] + aCopy[2]*bCopy[6]  + aCopy[3]*bCopy[5] + aCopy[4]*bCopy[4] + aCopy[5]*bCopy[3] + aCopy[6]*bCopy[2] + aCopy[7]*bCopy[1] + aCopy[8]*bCopy[0];
    s[9]  = cCopy[9]  + aCopy[0]*bCopy[9]  + aCopy[1]*bCopy[8] + aCopy[2]*bCopy[7]  + aCopy[3]*bCopy[6] + aCopy[4]*bCopy[5] + aCopy[5]*bCopy[4] + aCopy[6]*bCopy[3] + aCopy[7]*bCopy[2] + aCopy[8]*bCopy[1] + aCopy[9]*bCopy[0];
    s[10] = cCopy[10] + aCopy[0]*bCopy[10] + aCopy[1]*bCopy[9] + aCopy[2]*bCopy[8]  + aCopy[3]*bCopy[7] + aCopy[4]*bCopy[6] + aCopy[5]*bCopy[5] + aCopy[6]*bCopy[4] + aCopy[7]*bCopy[3] + aCopy[8]*bCopy[2] + aCopy[9]*bCopy[1] + aCopy[10]*bCopy[0];
    s[11] = cCopy[11] + aCopy[0]*bCopy[11] + aCopy[1]*bCopy[10] + aCopy[2]*bCopy[9] + aCopy[3]*bCopy[8] + aCopy[4]*bCopy[7] + aCopy[5]*bCopy[6] + aCopy[6]*bCopy[5] + aCopy[7]*bCopy[4] + aCopy[8]*bCopy[3] + aCopy[9]*bCopy[2] + aCopy[10]*bCopy[1] + aCopy[11]*bCopy[0];
    s[12] = aCopy[1]*bCopy[11]  + aCopy[2]*bCopy[10]  + aCopy[3]*bCopy[9]  + aCopy[4]*bCopy[8]  + aCopy[5]*bCopy[7]  + aCopy[6]*bCopy[6]  + aCopy[7]*bCopy[5]  + aCopy[8]*bCopy[4]  + aCopy[9]*bCopy[3]  + aCopy[10]*bCopy[2] + aCopy[11]*bCopy[1];
    s[13] = aCopy[2]*bCopy[11]  + aCopy[3]*bCopy[10]  + aCopy[4]*bCopy[9]  + aCopy[5]*bCopy[8]  + aCopy[6]*bCopy[7]  + aCopy[7]*bCopy[6]  + aCopy[8]*bCopy[5]  + aCopy[9]*bCopy[4]  + aCopy[10]*bCopy[3] + aCopy[11]*bCopy[2];
    s[14] = aCopy[3]*bCopy[11]  + aCopy[4]*bCopy[10]  + aCopy[5]*bCopy[9]  + aCopy[6]*bCopy[8]  + aCopy[7]*bCopy[7]  + aCopy[8]*bCopy[6]  + aCopy[9]*bCopy[5]  + aCopy[10]*bCopy[4] + aCopy[11]*bCopy[3];
    s[15] = aCopy[4]*bCopy[11]  + aCopy[5]*bCopy[10]  + aCopy[6]*bCopy[9]  + aCopy[7]*bCopy[8]  + aCopy[8]*bCopy[7]  + aCopy[9]*bCopy[6]  + aCopy[10]*bCopy[5] + aCopy[11]*bCopy[4];
    s[16] = aCopy[5]*bCopy[11]  + aCopy[6]*bCopy[10]  + aCopy[7]*bCopy[9]  + aCopy[8]*bCopy[8]  + aCopy[9]*bCopy[7]  + aCopy[10]*bCopy[6] + aCopy[11]*bCopy[5];
    s[17] = aCopy[6]*bCopy[11]  + aCopy[7]*bCopy[10]  + aCopy[8]*bCopy[9]  + aCopy[9]*bCopy[8]  + aCopy[10]*bCopy[7] + aCopy[11]*bCopy[6];
    s[18] = aCopy[7]*bCopy[11]  + aCopy[8]*bCopy[10]  + aCopy[9]*bCopy[9]  + aCopy[10]*bCopy[8] + aCopy[11]*bCopy[7];
    s[19] = aCopy[8]*bCopy[11]  + aCopy[9]*bCopy[10]  + aCopy[10]*bCopy[9] + aCopy[11]*bCopy[8];
    s[20] = aCopy[9]*bCopy[11]  + aCopy[10]*bCopy[10] + aCopy[11]*bCopy[9];
    s[21] = aCopy[10]*bCopy[11] + aCopy[11]*bCopy[10];
    s[22] = aCopy[11]*bCopy[11];
    s[23] = 0;

    carry[0]  = (s[0]  + (1<<20)) >> 21; s[1]  += carry[0];  s[0]  -= carry[0]  << 21;
    carry[2]  = (s[2]  + (1<<20)) >> 21; s[3]  += carry[2];  s[2]  -= carry[2]  << 21;
    carry[4]  = (s[4]  + (1<<20)) >> 21; s[5]  += carry[4];  s[4]  -= carry[4]  << 21;
    carry[6]  = (s[6]  + (1<<20)) >> 21; s[7]  += carry[6];  s[6]  -= carry[6]  << 21;
    carry[8]  = (s[8]  + (1<<20)) >> 21; s[9]  += carry[8];  s[8]  -= carry[8]  << 21;
    carry[10] = (s[10] + (1<<20)) >> 21; s[11] += carry[10]; s[10] -= carry[10] << 21;
    carry[12] = (s[12] + (1<<20)) >> 21; s[13] += carry[12]; s[12] -= carry[12] << 21;
    carry[14] = (s[14] + (1<<20)) >> 21; s[15] += carry[14]; s[14] -= carry[14] << 21;
    carry[16] = (s[16] + (1<<20)) >> 21; s[17] += carry[16]; s[16] -= carry[16] << 21;
    carry[18] = (s[18] + (1<<20)) >> 21; s[19] += carry[18]; s[18] -= carry[18] << 21;
    carry[20] = (s[20] + (1<<20)) >> 21; s[21] += carry[20]; s[20] -= carry[20] << 21;
    carry[22] = (s[22] + (1<<20)) >> 21; s[23] += carry[22]; s[22] -= carry[22] << 21;

    carry[1]  = (s[1]  + (1<<20)) >> 21; s[2]  += carry[1];  s[1]  -= carry[1]  << 21;
    carry[3]  = (s[3]  + (1<<20)) >> 21; s[4]  += carry[3];  s[3]  -= carry[3]  << 21;
    carry[5]  = (s[5]  + (1<<20)) >> 21; s[6]  += carry[5];  s[5]  -= carry[5]  << 21;
    carry[7]  = (s[7]  + (1<<20)) >> 21; s[8]  += carry[7];  s[7]  -= carry[7]  << 21;
    carry[9]  = (s[9]  + (1<<20)) >> 21; s[10] += carry[9];  s[9]  -= carry[9]  << 21;
    carry[11] = (s[11] + (1<<20)) >> 21; s[12] += carry[11]; s[11] -= carry[11] << 21;
    carry[13] = (s[13] + (1<<20)) >> 21; s[14] += carry[13]; s[13] -= carry[13] << 21;
    carry[15] = (s[15] + (1<<20)) >> 21; s[16] += carry[15]; s[15] -= carry[15] << 21;
    carry[17] = (s[17] + (1<<20)) >> 21; s[18] += carry[17]; s[17] -= carry[17] << 21;
    carry[19] = (s[19] + (1<<20)) >> 21; s[20] += carry[19]; s[19] -= carry[19] << 21;
    carry[21] = (s[21] + (1<<20)) >> 21; s[22] += carry[21]; s[21] -= carry[21] << 21;
    ...
    carry[0]  = s[0]  >> 21; s[1]  += carry[0];  s[0]  -= carry[0]  << 21;
    carry[1]  = s[1]  >> 21; s[2]  += carry[1];  s[1]  -= carry[1]  << 21;
    carry[2]  = s[2]  >> 21; s[3]  += carry[2];  s[2]  -= carry[2]  << 21;
    carry[3]  = s[3]  >> 21; s[4]  += carry[3];  s[3]  -= carry[3]  << 21;
    carry[4]  = s[4]  >> 21; s[5]  += carry[4];  s[4]  -= carry[4]  << 21;
    carry[5]  = s[5]  >> 21; s[6]  += carry[5];  s[5]  -= carry[5]  << 21;
    carry[6]  = s[6]  >> 21; s[7]  += carry[6];  s[6]  -= carry[6]  << 21;
    carry[7]  = s[7]  >> 21; s[8]  += carry[7];  s[7]  -= carry[7]  << 21;
    carry[8]  = s[8]  >> 21; s[9]  += carry[8];  s[8]  -= carry[8]  << 21;
    carry[9]  = s[9]  >> 21; s[10] += carry[9];  s[9]  -= carry[9]  << 21;
    carry[10] = s[10] >> 21; s[11] += carry[10]; s[10] -= carry[10] << 21;
    carry[11] = s[11] >> 21; s[12] += carry[11]; s[11] -= carry[11] << 21;
