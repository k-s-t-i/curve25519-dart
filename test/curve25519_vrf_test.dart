...
void main() {
  ...
  group('Curve25519', () {
    test('verify', () {
      ...
      var aliceKeyPair = KeyPair(
          PublicKey(aliceIdentityPublic), PrivateKey(aliceIdentityPrivate));
      ...
      for (var i = 0; i < aliceSignature.bytes.length; i++) {
        ...
        var modifiedSignature = Signature(
            ...
            publicKey: aliceKeyPair.publicKey,
            type: SignatureType.STANDARD);
        expect(!Curve25519().verify(aliceEphemeralPublic, modifiedSignature),
            isTrue);
      }
    });
    ...
    test('signature', () {
      ...
      message2[0] = 1;
      var signature =
          Curve25519().sign(keyPair, message1, SignatureType.STANDARD);
    ...
    test('agreement', () {
      ...
      var aliceKeyPair =
          KeyPair(PublicKey(alicePublic), PrivateKey(alicePrivate));
      var bobKeyPair = KeyPair(PublicKey(bobPublic), PrivateKey(bobPrivate));
      var sharedOne =
          Curve25519().calculateAgreement(aliceKeyPair, bobKeyPair.publicKey);
      var sharedTwo =
          Curve25519().calculateAgreement(bobKeyPair, aliceKeyPair.publicKey);

      expect(
          ListEquality().equals(shared, Int8List.fromList(sharedOne)), isTrue);
      expect(
          ListEquality().equals(shared, Int8List.fromList(sharedTwo)), isTrue);
    });
    ...
    test('largeSignatures', () {
      ...
      var signature1 =
          Curve25519().sign(keyPair, message1, SignatureType.STANDARD);
      var signature2 =
          Curve25519().sign(keyPair, message2, SignatureType.STANDARD);
      expect(() => Curve25519().sign(keyPair, message3, SignatureType.STANDARD),
          throwsException);
      ...
      var signatureVRF1 =
          Curve25519().sign(keyPair, message1, SignatureType.VRF);
      var signatureVRF2 =
          Curve25519().sign(keyPair, message2, SignatureType.VRF);
      expect(() => Curve25519().sign(keyPair, message3, SignatureType.VRF),
          throwsException);
